...
function manipulateUrl(url) {
  ...
  href.protocol = "https:";
  href.host = "newhost.com";
  href.searchParams.set("newParam", "newValue");
  href.searchParams.delete("oldParam");
  ...
  return {
    ...
    search: href.search,
    params: Array.from(href.searchParams),
  };
}
...
function searchParamsURL(url) {
  ...
  // Повертаємо словник
  const href = new URL(url);
  const URLSearchParams = href.searchParams;
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  for (const key of Object.keys(paramsObj)) {
    let urlObj = href.searchParams.set(key, paramsObj[key]);
  }
...
function deleteSearchParams(keys, url) {
  ...
  for (const key of Object.keys(keys)) {
    href.searchParams.delete(keys[key]);
  }
  return href.toString();
}
...
function createURLWithParams(params, url) {
  ...
  for (const key in params) {
    href.searchParams.set(key, params[key]);
  }
  return href.toString();
}
...
function checkURLParameters(url, params) {
  ...
  for (const element of params) {
    obj[element] = href.searchParams.has(element);
  }
  return obj;
}
...
function manipulateQuery(url, options) {
  ...
  const map = Object.fromEntries(options);

  if (map.append) {
    for (const [key, value] of map.append) {
      //    console.log(key);
      href.searchParams.append(key, value);
    }
  }

  // console.log(Object.fromEntries(options).delete);

  if (map.delete) {
    for (const key of map.delete) {
...
function sortUrlParams(url) {
  ...
  //const sortFn = (elem1, elem2) => elem1[0] - elem2[0];
  // => {
  //   if (elem1[0] < elem2[0]) {
  //     console.log(elem1[0] - elem2[0]);
  //   }
  // };
  arr.sort();
  console.log(arr);
...
function getURLValues(url) {
  ...
  const arr = Array.from(urlSearchParams.keys());
